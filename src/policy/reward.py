...
class RewardFunction:
    def __init__(self, config: dict):
        ...
        self.stage               = train_cfg.get('stage', 'transport_hold')
        ...
        self.w_carry             = float(reward_cfg.get('w_carry', 0.0))
        self.w_lift              = float(reward_cfg.get('w_lift', 4.0))
        self.w_target_xy         = float(reward_cfg.get('w_target_xy', 0.0))
        self.r_transport_success = float(reward_cfg.get('r_transport_success', 50.0))
        ...
        self.transport_stable_steps = int(reward_cfg.get('transport_stable_steps', 5))
        self.release_height_tol  = float(reward_cfg.get('release_height_tol', self.place_success_z))

        self._prev_grasped       = False
        self._prev_close_cmd     = False
        self._grasp_stable_count = 0
        self._transport_stable_count = 0
        self._grasp_bonus_paid   = False
        self._has_lifted         = False
        self._prev_obj_height    = None
        self._prev_place_dist    = None
    ...
    def reset(self, obs: dict):
        ...
        self._prev_obj_height = obj_height
    ...
    def compute(self, obs: dict, action: np.ndarray) -> dict:
        ...
        if self._prev_obj_height is None:
            self._prev_obj_height = obj_height
        if self._prev_place_dist is None:
            self._prev_place_dist = place_dist

        lift_progress  = obj_height - self._prev_obj_height
        carry_progress = self._prev_place_dist - place_dist
        ...
        target_xy_ready = place_dist < self.place_success_dist
        target_z_ready  = obj_height >= self.lift_thresh
        transport_ready = grasped and target_xy_ready and target_z_ready

        if transport_ready:
            self._transport_stable_count += 1
        else:
            self._transport_stable_count = 0
        ...
        if self.stage == 'reach_grasp':
        ...
        elif self.stage == 'transport_hold':
            if not grasped and not self._has_lifted:
                r_reach = r_align_xy + r_align_z

            if grasped:
                r_hold = self.w_hold
                r_lift = self.w_lift * max(lift_progress, 0.0)
                if obj_height >= self.micro_lift_height:
                    r_carry = self.w_carry * carry_progress
                    r_target_xy = -self.w_target_xy * place_dist

            success = self._transport_stable_count >= self.transport_stable_steps
            if success:
                r_succ = self.r_transport_success
            ...
            placed_released = (not grasped) and (place_dist < self.place_success_dist) and (place_z_err < self.release_height_tol)
            success = placed_released
            if success:
                r_succ = self.r_success
        ...
        total = r_reach + r_grasp + r_hold + r_carry + r_lift + r_target_xy + r_succ + r_drop + r_time + r_action + r_prem_close + r_switch
        ...
        return {
            ...
            'carry_progress':         float(carry_progress),
            'grasped':                grasped,
            ...
            'transport_stable_count': float(self._transport_stable_count),
            'has_lifted':             bool(self._has_lifted),
            'reach_success':          bool(reach_success),
            'lift_success':           bool(lift_success),
            'micro_lift_success':     bool(micro_lift_success),
            'target_xy_ready':        bool(target_xy_ready),
